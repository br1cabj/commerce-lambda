...
const migrate = async () => {
  try {
    ...
    const defaultOwner = await User.findOne({ role: "admin" }).sort({
      createdAt: 1,
    });

    if (!defaultOwner) {
      console.log(
        "No admin user found. Create an admin user first, then run this script.",
      );
    ...
    if (!defaultTenant) {
      defaultTenant = await Tenant.create({
        ...
        settings: {
          ...
          phone: "",
          address: "",
          paymentMethods: [{ type: "whatsapp", enabled: true, config: {} }],
          shippingMethods: [
      ...
      console.log(
        `Tenant created: ${defaultTenant.name} (${defaultTenant.slug})`,
      );
    } else {
