...
export const createCoupon = async (req, res) => {
  try {
    const { code, discountPercentage, pointsRequired, expiresAt, maxUses } =
      req.body;
...
export const validateCoupon = async (req, res) => {
  try {
    ...
    if (!couponFound) {
      return res
        .status(404)
        .json({ message: "Coupon does not exist or is misspelled." });
    }

    if (!couponFound.isActive) {
      return res
        .status(400)
        .json({ message: "This coupon has been deactivated." });
    }
    ...
    if (couponFound.maxUses && couponFound.usedCount >= couponFound.maxUses) {
      return res
        .status(400)
        .json({ message: "This coupon has reached its maximum uses." });
    }
