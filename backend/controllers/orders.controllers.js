...
const VALID_ORDER_STATUSES = ["Pendiente", "En Preparación", "Enviado", "Entregado", "Cancelado"];
...
export const createOrder = async (req, res) => {
    ...
    try {
        ...
        for (let item of products) {
            const productFound = await Product.findOne({ _id: item.product, tenantId: req.tenant._id, isDeleted: false }).session(session);
        ...
        if (couponCode) {
            const coupon = await Coupon.findOne({ code: couponCode.toUpperCase(), tenantId: req.tenant._id }).session(session);
...
export const getMyOrders = async (req, res) => {
    try {
        ...
        const orders = await Order.find({ tenantId: req.tenant._id, user: req.user.id })
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(Number(limit));

        const total = await Order.countDocuments({ tenantId: req.tenant._id, user: req.user.id });
