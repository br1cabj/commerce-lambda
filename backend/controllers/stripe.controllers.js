...
export const createStripeSession = async (req, res) => {
    try {
        ...
        for (let item of products) {
            const productFound = await Product.findOne({ _id: item.product, tenantId: tenant._id, isDeleted: false });
            if (!productFound) {
        ...
        if (couponCode) {
            const coupon = await Coupon.findOne({ code: couponCode.toUpperCase(), tenantId: tenant._id });
            if (coupon && coupon.isActive && (!coupon.expiresAt || new Date(coupon.expiresAt) > new Date()) && (!coupon.maxUses || coupon.usedCount < coupon.maxUses)) {
                ...
                lineItems.forEach(item => {
                    item.price_data.unit_amount = Math.round(item.price_data.unit_amount * discountFactor);
                });
...
export const handleStripeWebhook = async (req, res) => {
    try {
        ...
        if (tenantId) {
            ...
            tenant = await Tenant.findOne({ "settings.paymentMethods.type": "stripe" });
        }
        ...
        if (event.type === "checkout.session.completed") {
            ...
            const existingOrder = await Order.findOne({ externalReference: metadata.orderId });
            if (existingOrder) {
                return res.status(200).json({ received: true, message: "Order already created" });
            }
            ...
            try {
                ...
                if (metadata.couponCode) {
                    const coupon = await Coupon.findOne({ code: metadata.couponCode, tenantId: metadata.tenantId }).session(sessionDb);
                    if (coupon) {
                        coupon.usedCount += 1;
