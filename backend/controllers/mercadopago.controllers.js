    ...
    try {
        ...
        if (couponCode) {
            ...
            if (coupon && coupon.isActive && (!coupon.expiresAt || new Date(coupon.expiresAt) > new Date()) && (!coupon.maxUses || coupon.usedCount < coupon.maxUses)) {
                ...
                items.forEach(item => {
                    item.unit_price = Number((item.unit_price * discountFactor).toFixed(2));
                });
...
export const handleMercadoPagoWebhook = async (req, res) => {
    try {
        ...
        if (type === "payment" && data?.id) {
            ...
            const existingEvent = await WebhookEvent.findOne({ eventId: `mp_${paymentId}` });
            if (existingEvent) {
            ...
            if (payment.status === "approved") {
                ...
                try {
                    ...
                    if (metadata.couponCode) {
                        const coupon = await Coupon.findOne({ code: metadata.couponCode, tenantId: metadata.tenantId }).session(session);
                        if (coupon) {
                            coupon.usedCount += 1;
