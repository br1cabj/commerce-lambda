        ...
        <form onSubmit={createCoupon} className="bg-white rounded-xl shadow-sm border p-6 mb-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <input type="text" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} className="w-full px-4 py-2 rounded-lg border uppercase" placeholder="SAVE10" required />
            </div>
            <div>
              <label className="block text-sm font-bold mb-1">Discount %</label>
              <input type="number" value={discount} onChange={(e) => setDiscount(e.target.value)} className="w-full px-4 py-2 rounded-lg border" min="1" max="100" required />
            </div>
            <div>
              <label className="block text-sm font-bold mb-1">Points Required</label>
              <input type="number" value={points} onChange={(e) => setPoints(e.target.value)} className="w-full px-4 py-2 rounded-lg border" min="0" />
            </div>
          </div>
          <div className="flex gap-3">
            <button type="submit" className="px-6 py-2 rounded-lg font-semibold text-white" style={{ backgroundColor: config.theme.accentColor }}>Create</button>
      ...
      {loading ? (
        <div className="space-y-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bg-white rounded-xl shadow-sm border p-4 animate-pulse h-16" />
          ))}
        </div>
      ) : coupons.length === 0 ? (
        ...
        <div className="space-y-3">
          {coupons.map((coupon) => (
            <div key={coupon._id} className="bg-white rounded-xl shadow-sm border p-4 flex items-center justify-between">
              <div>
                <p className="font-bold text-lg">{coupon.code}</p>
                <p className="text-sm text-gray-500">
                  {coupon.discountPercentage}% discount
                  {coupon.pointsRequired > 0 && ` • Requires ${coupon.pointsRequired} points`}
                </p>
              </div>
              <div className="flex items-center gap-2">
                <span className={`px-2 py-1 rounded text-xs font-bold ${coupon.isActive ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-500"}`}>
                  {coupon.isActive ? "Active" : "Inactive"}
                </span>
                <button onClick={() => toggleCoupon(coupon._id)} className="p-2 rounded hover:bg-gray-100">
                  {coupon.isActive ? <ToggleRight className="h-6 w-6 text-green-500" /> : <ToggleLeft className="h-6 w-6 text-gray-400" />}
                </button>
                <button onClick={() => deleteCoupon(coupon._id)} className="p-2 rounded hover:bg-red-50 text-red-500">
