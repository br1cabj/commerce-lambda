...
interface Order {
  ...
  shippingAddress: { street: string; number: string; city: string; province: string };
  trackingCode: string;
  totalAmount: number;
}
...
export default function AdminOrdersPage() {
  ...
  const updateStatus = async (orderId: string, status: string, trackingCode: string) => {
    if (!config) return;
    try {
          ...
          {orders.map((order) => (
            <div key={order._id} className="bg-white rounded-xl shadow-sm border p-6">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {new Date(order.createdAt).toLocaleDateString()} - {order.user?.name || "Deleted User"}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  {order.products.map((p, i) => (
                    <p key={i} className="text-sm">x{p.quantity} - ${p.price.toLocaleString()}</p>
                  ))}
                </div>
                <div>
                  <p className="text-sm text-gray-500">Ship to</p>
                  <p className="text-sm">{order.shippingAddress.street} {order.shippingAddress.number}, {order.shippingAddress.city}</p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 pt-4 border-t">
                <select
                  value={order.status}
                  onChange={(e) => updateStatus(order._id, e.target.value, order.trackingCode)}
                ...
                <input
                  ...
                  onBlur={(e) => updateStatus(order._id, order.status, e.target.value)}
                />
